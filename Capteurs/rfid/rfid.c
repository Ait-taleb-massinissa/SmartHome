...
static const char *AUTHORIZED_UID = "9289851D";
...
void rfid_task(void *pvParameters)
{
	...
	while (1)
	{
		...
		{
			if (PICC_Select(spi, &uid, 0) == 0) // success
			{
				...
				{
					pos += sprintf(uid_str + pos, "%02X", uid.uidByte[i]);
				}

				if (strcmp(uid_str, AUTHORIZED_UID) == 0)
				{
					printf("ACCESS GRANTED\n");
					servo_ouvrir(GPIO_NUM_27);
					vTaskDelay(5000 / portTICK_PERIOD_MS);
					servo_fermer(GPIO_NUM_27);
				}
				else
				{
